...
const presetEnv = require("postcss-preset-env"); // postcss-cssnext の代替
...
const cssSass = () => {
  return src(srcPath.css) //コンパイル元
    ...
    .pipe(
      postcss([
        presetEnv({ stage: 3, overrideBrowserslist: browsers }), // postcss-preset-env を使用
        combineMq(), // メディアクエリをまとめる
      ])
    )
